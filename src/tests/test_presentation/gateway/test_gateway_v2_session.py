...
async def test_v2_connect_sends_workspace_shell_before_persisted_sessions(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("voidx.agent.adapters.persistence.session_repository.list_sessions", slow_list_sessions)
...
async def test_v2_broadcast_turn_terminal_events():
    from voidx.presentation.output.events.schema import TurnCancelled, TurnCompleted, TurnFailed
...
async def test_v2_snapshot_preserves_background_failed_thread_status():
    from voidx.presentation.output.events.schema import TurnFailed
...
async def test_v2_snapshot_refreshes_background_thread_persisted_metadata(tmp_path: Path):
    import voidx.persistence.sqlite as store
    from voidx.agent.adapters.persistence.session_repository import MessageRow, create_session, save_message
