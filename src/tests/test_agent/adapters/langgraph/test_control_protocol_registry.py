"""Control protocol registry: each runtime profile declares which graph-level protocol tool set it uses."""
...
def test_unknown_protocol_falls_back_to_turn() -> None:
    ...
    assert isinstance(resolve_control_protocol(profile), TurnToolProtocol)


def test_resolve_none_profile_falls_back_to_turn() -> None:
    assert isinstance(resolve_control_protocol(None), TurnToolProtocol)
