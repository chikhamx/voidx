...
class TestSubagentTodoEmit:
    @pytest.mark.asyncio
    async def test_subagent_todo_tool_uses_queued_emit_with_agent_id(self):
        ...
        with patch("voidx.agent.adapters.langgraph.runtime.subagent.todo_updated_event", return_value=fake_event) as mock_make:
            ...
            fake_events.emit.assert_awaited_once_with(fake_event)
    ...
    @pytest.mark.asyncio
    async def test_subagent_skips_queued_emit_when_event_is_none(self):
        from voidx.agent.adapters.langgraph.runtime import subagent as sub_mod
        ...
        with patch("voidx.agent.adapters.langgraph.runtime.subagent.todo_updated_event", return_value=None):
            ...
            fake_events.emit.assert_not_awaited()


def test_todo_read_result_does_not_create_updated_event():
    result = ToolResult(
        output="Todo all (1 item)",
        metadata={
            "todo_op": "read",
            "todo_items": [{"id": "task", "content": "inspect", "status": "active"}],
            "todo_summary": "0/1 done · 1 active · 0 pending",
        },
    )

    assert todo_updated_event(result, agent_id=3) is None
