...
@pytest.mark.asyncio
async def test_loop_update_requires_loop_controller() -> None:
    tool = LoopUpdateTool()
...
@pytest.mark.asyncio
async def test_loop_update_submits_continue_decision() -> None:
...
@pytest.mark.asyncio
async def test_loop_update_ignores_dynamic_delay_for_fixed_loop() -> None:
...
@pytest.mark.asyncio
async def test_loop_update_stop_is_terminal_without_delay() -> None:
    controller = FakeLoopController()
    tool = LoopUpdateTool()
    ctx = ToolContext(workspace="/tmp/workspace", loop_controller=controller)

    result = await tool.execute({"outcome": "stop", "summary": "finished"}, ctx)

    assert result.metadata["outcome"] == "stop"
    assert result.metadata["terminal"] is True
    assert result.metadata["next_delay_seconds"] is None
