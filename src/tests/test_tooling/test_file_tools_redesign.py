...
from voidx.tooling.builtin.file import FileReadTool, ManageTool, WriteTool
...
class TestManageToolDirectoryOps:
    ...
    async def test_manage_delete_directory_clears_only_descendant_tracking(self, tmp_path):
        ...
        assert child_key not in ctx.file_state.mtimes
        assert child_key not in ctx.file_state.read_coverage
        assert sibling_key in ctx.file_state.mtimes
        assert sibling_key in ctx.file_state.read_coverage
    ...
    async def test_manage_move_directory_clears_source_and_destination_tracking(self, tmp_path):
        ...
        assert old_dest_key not in ctx.file_state.mtimes
        assert old_dest_key not in ctx.file_state.read_coverage
    ...
    async def test_manage_delete_directory_rejects_extra_allowed_root(self, tmp_path):
        ...
        ctx = ToolContext(
            ...
            authorization_service=AuthorizationRuntime(write_dirs=[str(extra_root)]),
        )
    ...
    async def test_manage_move_directory_clears_tracking_when_move_fails_after_overwrite(
        ...
        monkeypatch.setattr("voidx.tooling.policy.filesystem.safe_path.os.replace", fail_move)
        ...
        assert source_key not in ctx.file_state.mtimes
        assert source_key not in ctx.file_state.read_coverage
        assert dest_key not in ctx.file_state.mtimes
        assert dest_key not in ctx.file_state.read_coverage
    ...
    async def test_manage_delete_directory_clears_tracking_after_partial_failure(
        ...
        monkeypatch.setattr("voidx.tooling.policy.filesystem.safe_path._remove_tree", partially_delete)
