...
async def test_write_full_create_formats_and_failure_preserves_edit(tmp_path):
    ...
    registry = build_registry(lsp_operations=FormattingManager(tmp_path, "print(1)\n"))
    success_ctx = ToolContext(workspace=str(tmp_path))
    ...
    registry = build_registry(lsp_operations=FailingManager())
    failed_ctx = ToolContext(workspace=str(tmp_path))
