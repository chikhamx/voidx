...
from voidx.tooling.adapters.permission.in_memory_state import PathGrantLockManager
...
async def test_context_grants_are_refreshed(tmp_path):
    ...
    first = await build_registry().execute_tool("read", {"file_path": str(target)}, ctx)
    second = await build_registry().execute_tool("read", {"file_path": str(target)}, ctx)
...
def test_permission_not_ready_blocks_all_authorization_entries(tmp_path):
    ...
    context = service.context_for(workspace=str(tmp_path / "workspace"))
...
async def test_tool_context_get_access_grants_fails_closed_when_not_ready(tmp_path):
    ...
    result = await build_registry().execute_tool("read", {"file_path": str(external)}, ctx)
...
async def test_manage_external_path_uses_existing_external_grant(tmp_path):
    ...
    result = await build_registry().execute_tool("manage", {"op": "create", "paths": str(target)}, ctx)
