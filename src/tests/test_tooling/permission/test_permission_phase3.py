...
def test_authorized_path_is_unforgeable(tmp_path):
    from voidx.tooling.policy.filesystem.safe_path import AuthorizedPath
...
async def test_read_binary_probe_symlink_swap_fails_closed(tmp_path, monkeypatch):
    import voidx.tooling.builtin.file.read as read_module
    ...
    result = await build_registry().execute_tool("read", {"file_path": "target.txt"}, ToolContext(workspace=str(workspace)))
...
def test_safe_path_delete_tree_rejects_child_symlink_race(tmp_path, monkeypatch):
    import shutil
    import voidx.tooling.policy.filesystem.safe_path as safe_path
    from voidx.tooling.policy.filesystem.safe_path import SafePathExecutor
