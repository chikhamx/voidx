...
from voidx.lsp.domain.schema import LspLocation, LspPosition, LspRange
...
from voidx.tooling.application.execution import AuthorizationRuntime, FileToolContext
...
def test_execution_lease_token_is_unforgeable():
    from voidx.tooling.application.permission_service import ExecutionLease
...
async def test_agent_tool_passes_subagent_permission_snapshot(tmp_path):
    ...
    ctx = AgentToolExecutionContext(
        ...
        runtime=AgentToolRuntime(
            ...
            revocation_epoch=lambda: service.revocation_epoch,
        ),
    )
...
async def test_lsp_filters_external_locations(tmp_path):
    ...
    ctx = FileToolContext(
        ...
        authorization_service=AuthorizationRuntime(access_grants_reader=lambda: AccessGrants()),
    )

    result = await LspTool(manager, ctx.authorization_service).execute(
