...
from voidx.tooling.policy.permission.wildcard import match
from voidx.tooling.policy.permission.evaluate import evaluate, from_config, merge
from voidx.tooling.domain.permission import Rule, Ruleset
from voidx.tooling.policy.permission.rules import BASIC_RULES
...
def test_permission_engine_classifies_basic_capabilities():
    ...
    implement_agent = classify_tool_call(project_agent_tool_call({"name": "agent", "args": {"agent": "implement"}}))
    assert implement_agent.capability == PermissionCapability.AGENT_IMPLEMENT
    assert implement_agent.pattern == "implement"
    mode_implement_agent = classify_tool_call(project_agent_tool_call({"name": "agent", "args": {"name": "voidx", "mode": "implement"}}))
