...
def test_permission_read_only_status():
    from voidx.tooling.policy.permission.rules import _is_read_only_git_tool_command
...
def test_git_is_read_only_reflog():
    ...
    assert is_read_only_subcommand("reflog", ["show"]) is True
    assert is_read_only_subcommand("reflog", ["list"]) is True
    assert is_read_only_subcommand("reflog", ["expire", "--all"]) is False


def test_git_is_read_only_public_api():
    from voidx.tooling.builtin.git import is_git_read_only
