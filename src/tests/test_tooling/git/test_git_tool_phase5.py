...
async def test_git_external_path_must_be_repo_root(tmp_path: Path):
    ...
    result = await GitTool().execute(
        ...
        ToolContext(
            ...
            authorization_service=AuthorizationRuntime(access_grants_reader=lambda: AccessGrants.from_parts(readable_dirs=[str(repo)])),
        ),
    )
...
async def test_git_requires_linked_worktree_common_dir(tmp_path: Path):
    ...
    result = await GitTool().execute(
        ...
        ToolContext(
            workspace=str(workspace),
            authorization_service=AuthorizationRuntime(access_grants_reader=lambda: AccessGrants.from_parts(readable_dirs=[str(linked)])),
        ),
    )
...
async def test_git_nested_config_include_requires_grant(tmp_path: Path):
    ...
    result = await GitTool().execute(
        ...
        ToolContext(
            workspace=str(workspace),
            authorization_service=AuthorizationRuntime(access_grants_reader=lambda: AccessGrants.from_parts(
...
async def test_git_external_child_path_denied_before_repo_discovery(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    from voidx.tooling.builtin.git import tool as git_tool_mod
    ...
    monkeypatch.setattr(git_tool_mod, "discover_repo", fail_discover)
