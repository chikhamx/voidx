...
@pytest.mark.asyncio
async def test_gitrun_process_preserves_structured_timeout(monkeypatch, tmp_path):
    class FakeProcess:
        returncode = None
        pid = 12345
    ...
    from voidx.tooling.builtin.git import process as git_process_mod
    ...
    result = await git_process_mod.run_process(
        ["git", "status"],
        cwd=str(tmp_path),
        timeout=0.01,
    )
