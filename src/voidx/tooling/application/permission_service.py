...
from voidx.tooling.policy.filesystem.grants import delta_for_grant
from voidx.tooling.policy.permission.session_rules import format_session_rule
...
class PermissionService:
    ...
    def state_revision(self) -> int:
        return self._state.state_revision

    @property
    def permissions_revision(self) -> int:
        return self._state.permissions_revision

    @property
    def revocation_epoch(self) -> int:
        return self._state.revocation_epoch

    @property
    def ai_approval_count(self) -> int:
        return self._state.ai_approval_count

    @ai_approval_count.setter
    def ai_approval_count(self, value: int) -> None:
        self._state.set_ai_approval_count(value)

    # ── session whitelist management ─────────────────────────────────────

    def allow(self, tool: str) -> None:
        """Pre-approve a tool for the entire session. No more prompts."""
        self._state.set_session_rule(tool, allow=True)
    ...
    def deny(self, tool: str) -> None:
        ...
        self._state.set_session_rule(tool, allow=False)
    ...
    def set_permission_mode(self, preset: str) -> None:
        if self._state.has_active_leases():
    ...
    def get_access_grants(self) -> AccessGrants:
        return AccessGrants.from_parts(
            ...
            extra_grants=self._state.grants_snapshot(),
    ...
    async def add_grant(
        ...
        *,
        ...
        async with self._state.commit_guard():
            ...
            applied = self._state.add_grant(grant)
            return GrantUpdateResult(
                persistent=grant.persistence == "persistent",
                ok=True,
                committed=True,
    ...
    async def acquire_grant_targets(self, paths, *, final_paths=None) -> object:
        return await self._state.acquire_grant_targets(paths, final_paths=final_paths)

    async def acquire_execution_lease(self) -> ExecutionLease:
        return self._state.acquire_execution_lease()
    ...
    def _clear_runtime_grants(self) -> None:
        self._state.clear_runtime_grants_and_advance()

    def has_active_execution_lease(self, lease: ExecutionLease) -> bool:
        return self._state.has_active_execution_lease(lease)

    def _release_execution_lease(self, lease: ExecutionLease) -> None:
        self._state.release_execution_lease(lease)

    def inc_ai_approval_count(self) -> None:
        self._state.increment_ai_approval_count()
    ...
    def _session_short(self) -> str:
        session_allow, session_deny = self._state.session_rules()
        if session_allow and not session_deny:
            return f"+{len(session_allow)}"
        if session_deny and not session_allow:
            return f"-{len(session_deny)}"
        if session_allow and session_deny:
            return f"+{len(session_allow)}/-{len(session_deny)}"
        return "default"
    ...
    def show_rules(self) -> str:
        ...
        if session_allow:
            lines.append(f"  [green]Session allow:[/green] {', '.join(sorted(format_session_rule(rule) for rule in session_allow))}")
        if session_deny:
            lines.append(f"  [red]Session deny:[/red] {', '.join(sorted(format_session_rule(rule) for rule in session_deny))}")
    ...
    def clear_session_permissions(self) -> None:
        ...
        self._state.clear_session_permissions()
    ...
    def context_for(
        ...
        *,
        ...
        mode = "plan" if plan_mode else (interaction_mode or "auto")
        return PermissionContext(
            ...
            permission_state_ready=self.permission_state_ready,
            session_allow=self._state.session_rules()[0],
            session_deny=self._state.session_rules()[1],
