"""MCP server connection lifecycle and catalog orchestration.

Server configurations are supplied by the composition root. All connections
start in parallel, and one failure does not block the others.
"""
...
class McpManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._server_configs = list(configs)
        self._catalog = McpCatalog()
        self._description_generator = description_generator
        self._generated_descriptions: dict[str, str] = {}
        self._description_cache = McpDescriptionCache(workspace) if workspace is not None else None
    ...
    async def _start_servers(self, configs: list[McpServerConfig]) -> list[tuple[str, McpClient]]:
        ...
        async def try_start(sc: McpServerConfig) -> tuple[str, McpClient] | None:
            ...
            client = self._client_factory(sc)
            try:
                await client.start()
