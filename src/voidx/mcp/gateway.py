...
class McpGatewayTool(BaseTool):
    ...
    def _execute_list(self, inp: McpInput) -> ToolResult:
        statuses = {s.name: s for s in self._manager.statuses()}
        catalog = {e.name: e for e in self._manager.catalog_snapshot()}
    ...
    def _execute_load(self, inp: McpInput) -> ToolResult:
        ...
        status = next((s for s in self._manager.statuses() if s.name == inp.server), None)
        if status is None:
            known = sorted(s.name for s in self._manager.statuses())
        ...
        entry = next((e for e in self._manager.catalog_snapshot() if e.name == inp.server), None)
        tools = list(entry.tools) if entry is not None else []
        if inp.tool:
    ...
    async def _execute_call(self, inp: McpInput) -> ToolResult:
        ...
        tool_def = self._manager.tool_def(inp.server, inp.tool)
        if tool_def is None:
            entry = next((e for e in self._manager.catalog_snapshot() if e.name == inp.server), None)
        ...
        try:
            result = await self._manager.call_tool(inp.server, inp.tool, validated.arguments or {})
