...
class ModeCommandsMixin:
    def _set_interaction_mode(self, mode: str) -> None:
        ...
        self.host.ui.print(f"[dim]Mode set to [cyan]{labels.get(parsed, parsed.value)}[/cyan]{suffix}[/dim]")
    ...
    async def _goal(self, arg: str) -> None:
        ...
        if text == "status":
            ...
            if status is None:
                self.host.ui.print("[dim]/goal is not active.[/dim]")
                return
            self.host.ui.print(
        ...
        if text == "stop":
            ...
            self.host.ui.print("[dim]/goal stopped.[/dim]" if stopped else "[dim]/goal is not active.[/dim]")
            return
        objective, acceptance = _parse_goal_args(text)
        if not objective or not acceptance:
            self.host.ui.print("Usage: /goal <objective> --accept <acceptance condition>")
            return
    ...
    def _usage(self) -> None:
        ...
        if stats is None:
            self.host.ui.print("[dim]No usage data available.[/dim]")
            return
    ...
    def _log(self, arg: str) -> None:
        ...
        if not parts:
            ...
            self.host.ui.print(f"[dim]log exchange {ex}, diagnostic {di}[/dim]")
            return

        target = parts[0].lower()
        if target not in ("exchange", "diagnostic"):
            self.host.ui.error("Usage: /log [exchange|diagnostic] [on|off]")
            return
    ...
    async def _show_diff(self) -> None:
        ...
        if stat:
            self.host.ui.print(f"[bold]Changes:[/bold]\n{stat}\n")
            diff_text = git_diff(workspace)
            if diff_text:
                self.host.ui.diff(diff_text)
            else:
                self.host.ui.print("[dim]No diff content.[/dim]")
        else:
            self.host.ui.print("[dim]No changes in working tree.[/dim]")
