...
class PlanCheckpointTool:
    ...
    async def execute(self, args: dict, ctx: ToolContext) -> ToolResult:
        ...
        event_ui_active = _emit_checkpoint_shown(ctx.runtime.events, checkpoint_id, inp)
        ...
        if decision == "modified":
            scope_response = await ctx.runtime.interaction(UserInteraction(
...
def _emit_checkpoint_shown(
    ...
    if publisher is None or not publisher.is_running:
        return False
    publisher.emit(CheckpointPromptShown(
...
def _emit_checkpoint_decision(
    ...
    *,
    ...
    if publisher is None or not publisher.is_running:
        return
    publisher.emit(CheckpointDecisionSubmitted(
        checkpoint_id=checkpoint_id,
        decision=decision,
        label=label,
        response=response,
