...
class AgentTool:
    id = "agent"
    description = (
        "Start one isolated child agent for an independent task or well-scoped subtask when either it needs an independent context "
        "or it can proceed in parallel with the parent without overlapping the parent's active scope. "
        "The child does not inherit the caller's conversation history; return its run_id to control it."
    )
    ...
    async def execute(self, args: dict, ctx: ToolContext) -> ToolResult:
        ...
        try:
            ...
            run = await ctx.runtime.subagent_transport.spawn(
                session_id=ctx.session_id,
                parent_run_id=ctx.runtime.run_id,
...
def _runner_permission_kwargs(runner, ctx: ToolContext) -> dict[str, object]:
    if ctx.runtime.access_grants is None or ctx.runtime.revocation_epoch is None:
        return {}
    try:
        params = inspect.signature(runner).parameters
    except (TypeError, ValueError):
    ...
    return {
        "permission_snapshot": SubagentPermissionSnapshot.from_parts(
            ctx.runtime.access_grants(),
            ctx.runtime.revocation_epoch(),
            current_revocation_epoch=ctx.runtime.revocation_epoch,
        )
    }
