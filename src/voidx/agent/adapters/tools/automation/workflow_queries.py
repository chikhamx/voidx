...
def _workflow_dag(ctx: ToolContext) -> WorkflowDAG | None:
    return getattr(ctx.runtime, "workflow_dag", None)
...
def _select_advance_run(
    ...
    *,
    ...
    if requested:
        ...
        matched = _match_condition(condition, selected.name, dag)
        if matched is None:
            return None, None, _invalid_exit_guidance(condition, [selected], dag)
        return selected, matched, None

    candidates: list[tuple[WorkflowRunState, str]] = []
    for run in active:
        matched = _match_condition(condition, run.name, dag)
    ...
    return None, None, _invalid_exit_guidance(condition, active, dag)


def _invalid_exit_guidance(
    condition: str,
    active: list[WorkflowRunState],
    dag: WorkflowDAG,
) -> ToolResult:
...
def _match_node(name: str, dag: WorkflowDAG) -> str | None:
    normalized = name.strip().lower()
    for node in WorkflowService(dag).nodes():
        if node.name.strip().lower() == normalized:
            return node.name
    return None


def _match_condition(condition: str, workflow: str, dag: WorkflowDAG) -> str | None:
    normalized = condition.strip().lower()
    if normalized == workflow_terminal_condition(dag):
        return None
    for edge in workflow_edges(workflow, dag):
        if edge.condition.strip().lower() == normalized:
            return edge.condition
    return None


def _available_nodes(dag: WorkflowDAG) -> list[str]:
    return [node.name for node in WorkflowService(dag).nodes()]


def _available_exits(active: list[WorkflowRunState], dag: WorkflowDAG) -> list[str]:
    lines: list[str] = []
    for run in active:
    ...
    if active:
        lines.append(
            f"{workflow_terminal_condition(dag)} -> {workflow_terminal_description(dag)}"
        )
    return lines


def _suggested_advance_call(active: list[WorkflowRunState], dag: WorkflowDAG) -> str:
    for run in active:
        edges = workflow_edges(run.name, dag)
        if edges:
            edge = edges[0]
