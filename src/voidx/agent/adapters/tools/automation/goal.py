...
async def _submit_init(inp: GoalInput, ctx: ToolContext) -> ToolResult:
    controller = ctx.runtime.goal_intake
    if ctx.runtime.goal_phase not in {"intake", "idle"} or controller is None:
        return ToolResult(
...
async def _submit_decision(inp: GoalInput, ctx: ToolContext) -> ToolResult:
    ...
    controller = ctx.runtime.goal_control
    if ctx.runtime.goal_phase != "evaluator" or controller is None:
        return ToolResult(
