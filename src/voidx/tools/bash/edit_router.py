...
from voidx.tools.bash.hint.search import _SED_LINE_DELETE, _SED_RANGE_DELETE, _sed_split
from voidx.tools.shell.common import RouteHint, build_hint_result
...
async def maybe_route_sed_edit(command: str, ctx: ToolContext, source: str) -> ToolResult | None:
    ...
    if ctx.tool_registry is None or ctx.tool_registry.get("read") is None or ctx.tool_registry.get("replace") is None:
        return build_hint_result(command, hint, "Bash")
...
async def _route_sed_substitution(
    ...
    result = await ctx.tool_registry.execute_tool("replace", tool_args, ctx)
...
async def _read_managed_lines(path: str, start: int, end: int, ctx: ToolContext) -> list[str] | ToolResult:
    ...
    result = await ctx.tool_registry.execute_tool("read", read_args, ctx)
