...
3. **Load the template** — read the template file at `src/voidx/data/templates/{doc_type}.md` (inside the voidx package) to get the document skeleton. Fill in each section: replace `{placeholder}` fields with actual content, replace `<!-- guidance comments -->` with the corresponding section content. If information is insufficient for a field, mark it `[TBD]`. Never fabricate content.
