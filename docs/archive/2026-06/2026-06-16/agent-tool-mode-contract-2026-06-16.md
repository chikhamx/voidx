...
This mode routing table is authoritative for child-agent normalization. It does
not use `DEFAULT_WORKFLOW_DAG.goal_map`, because `goal_map` resolves top-level
user goals while the `agent` tool is normalizing an already-selected child task.

| mode | goal.type | plan.join | plan.leave | Notes |
|------|-----------|-----------|------------|-------|
| `inspect` | `inspect` | `review` | `review` | Intentional child-only routing override. Top-level `goal_map` currently sends inspect to `brainstorm`, but child inspection needs bounded read/search work, not user-facing design exploration. This temporary mapping can be replaced by a dedicated `inspect` workflow node later. Must not require review verdict semantics unless preset resolves to review. |
...
- `result_preset` must be valid for `mode`, unless it is `auto`.
- The existing rule `goal.type == "review" and plan.join != "review"`
  must be removed.
...
- `mode=inspect` does not consult `DEFAULT_WORKFLOW_DAG.goal_map`.
- `mode=feedback` normalizes to `goal.type=review`, `plan.join=feedback`, `plan.leave=verify` and is not rejected by the existing review-goal join check.
...
`NormalizedAgentDelegation` should contain:
...
Validation should operate on the normalized mode contract instead of assuming
every `GoalType.REVIEW` task must enter the `review` node. That assumption is
not true once `feedback` is a first-class child-agent mode.
