...
```python
# 只接受最后一条消息作为 AI 最终文本回复；不要向前搜索旧 AIMessage
last_msg = final["messages"][-1] if final["messages"] else None
if not isinstance(last_msg, AIMessage) or last_msg.tool_calls:
    return

assistant_text = extract_text(last_msg).strip()
...
- **终止消息约束**：只在本轮最后一条消息是没有 `tool_calls` 的 `AIMessage` 且能提取到非空文本时记录 `TurnExchange`；如果最后一条是 `ToolMessage`、带工具调用的 `AIMessage` 或其他消息类型，则跳过该轮
...
- 多轮工具调用后，不能用倒序搜索得到的旧 AIMessage 作为最终文本回复
- `extract_text()` 对 structured content 只提取 `type=text` 的部分，可能丢失上下文
- compaction 后消息列表被裁剪，`final["messages"]` 中可能没有可用的最终 AI 回复

**决策**：不再使用 `latest_ai_message()` 向前搜索。只检查 `final["messages"][-1]`：最后一条必须是没有 `tool_calls` 的 `AIMessage` 且文本非空才记录；否则跳过该轮 `TurnExchange`。宁可少记录一轮，也不要把工具结果、中间 AIMessage 或旧回复传给 goal resolver。
...
- `store.py` 直接 drop `recent_user_texts_json` 数据库列，不保留兼容字段
...
| `src/voidx/memory/store.py` | 直接 drop `recent_user_texts_json` 数据库列，不保留兼容字段 |
