...
- TUI slash command palette 需要暴露 `/mcp disable` 和 `/mcp enable`，避免功能存在但 `/` 候选不可发现。
...
`set_mcp_server_disabled()` 内部与 `delete_mcp_server()` 保持一致：调用 `clear_web_routes_for_server(name)` 时不立即保存，由外层 `_save()` 统一落盘，避免一次操作写两次 settings。
...
| Test | Description |
|------|-------------|
| `test_mcp_enable_disable_commands_are_in_palette` | `/` command palette 暴露 `/mcp enable|disable` |
| `test_settings_set_mcp_server_disabled_clears_web_routes` | settings 能切换 disabled 字段，并在 disable 时清理指向该 server 的 web routes |
...
| MCP manager lifecycle coverage | `McpManager.restart_all()` 现有行为会 `unregister_prefix("mcp__")` 并跳过 disabled server，slash tests 通过 restart 调用链覆盖 |
...
| `test_tavily_mcp_restart_skips_when_manager_unavailable` | `_mcp_manager=None` 时静默跳过 restart |
...
- TUI slash command palette 已补 `/mcp enable`、`/mcp disable` 候选。
