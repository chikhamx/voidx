...
6. Current Task State（拼到最新一条消息上，每轮 LLM 调用前迁移）
7. Inline Compaction Guide（HumanMessage，如有，追加到末尾）
```

注：Todo 已合并进 Task State，不再独立；`## User Message` 改为 `## Task Context`（见下方）；Goal Resolution Guide 由独立 goal resolver 的轻量提示词替代，不再注入主对话消息流。
...
7. **AIMessage / ToolMessage content 格式**：AIMessage 的 content 可能是 list（含 tool_calls 的结构化内容），`_prepend_task_context()` 和 `_strip_turn_overlay()` 需要处理 list 格式：拼接时在 list 头部插入 text block，剥离时移除头部 text block。对 AIMessage / ToolMessage 加 overlay 时只改 `content`，保持 `tool_calls`、`tool_call_id`、`id` 等结构字段原样
8. **Goal Resolution Guide 移除**：当前代码中的 Goal Resolution Guide 是旧的主循环分类提示。重构后 goal resolver 已有独立轻量提示词和结构化输出，主对话消息流不再注入该 guide

### 历史消息说明

历史对话位于 Runtime State 之后。由 AiMessage、ToolMessage 和历史 HumanMessage 交替组成：
...
### Goal Resolution Guide 移除说明

Goal Resolution Guide 不再注入主对话消息流。原因：

1. Goal Resolver 是每轮开始前的独立轻量分类器，已有专用提示词和 `GoalResolution` 结构化输出
2. 主对话消息流已经通过 Current Task State 获得 resolver 产出的 intent、goal 和 workflow route
3. 继续注入 guide 会让主 LLM 重复做分类判断，增加 token 并可能与 resolver 结果产生语义冲突

实现上删除 `include_goal_resolution_guide` 的生成、缓存、注入逻辑，并更新相关测试。
...
5. **消息构建**：每个 `TurnExchange` 生成 `HumanMessage` + `AIMessage`，最后追加当前用户输入 `HumanMessage`；以 `goal-resolver-refactor-2026-06-16.md` 为准，不把 AI 回复包装成 `HumanMessage("Assistant: ...")`

Goal Resolver 的上下文重构会替代主对话消息流中的 Goal Resolution Guide。主对话 LLM 只消费 resolver 写入 Task State 的结果，不再看到单独的分类 guide。
...
| 文件 | 改动 |
|------|------|
| `src/voidx/agent/runtime_context.py` | _current_task_state() 字段重构（移除 Latest user request 等）、_render_envelope() 字段重构、Runtime State 拆为独立消息、_prepend_task_context() 将 `## User Message` 改为 `## Task Context` 并支持所有消息类型、_strip_turn_overlay() 扩展到 AIMessage/ToolMessage、compile_messages() 中 _last_user_index 改为末尾定位、删除 Goal Resolution Guide 渲染/缓存/注入逻辑、删除废弃函数 |
| `src/voidx/agent/graph/core.py` | 移除 current_todo_context_message 的生成和追加、Runtime State 独立消息的组装、移除 include_goal_resolution_guide 传参 |
...
| `tests/test_agent/test_runtime_context.py` | 更新测试用例，覆盖 Task State 拼接到 AIMessage/ToolMessage 的场景；删除 Goal Resolution Guide 插入/复用测试 |
