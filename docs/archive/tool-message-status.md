...
失败判定只使用 `_tool_result_ok()` / `result_ok()` 的结构化规则，不对 `ToolResult.output` 做文本语义猜测。也就是说，若某个工具返回了类似失败的自然语言文本，但没有设置 `metadata.error` / `metadata.blocked` / `metadata.timeout` / 非零 `exit_code`，本设计不会把它标成 error；这类工具需要单独 audit 并补 metadata。
...
Sanitization 只清理协议层 tool exchange：失败的 assistant tool-use 表示和对应 `ToolMessage`。如果模型已经在后续 `AIMessage` 中用自然语言总结了错误、解释了原因或选择了替代方案，这些 assistant 文本会保留在历史中。
...
所有 `status` 值通过 low-level shared helper 归一化：

```python
# voidx/llm/message_status.py
...
放在 `voidx/llm/message_status.py`，避免 `memory/session.py` 为了 JSONL hydration 反向依赖 `voidx.agent.*`。如果后续发现只有 memory 层使用，也可以放在 `voidx/memory/message_status.py`；不要放在 `voidx/agent/message_rows.py` 或 sanitizer 模块里供 memory import。
...
| 工具返回失败文本但 metadata 未标错 | 保持 success；不做文本语义猜测，另行修工具 metadata |
| 后续 assistant 已总结失败原因 | 保留 assistant 自然语言总结，只清理协议层 tool exchange |
...
- `test_anthropic_adapter_receives_is_error_for_error_tool_message`：`ToolMessage(status="error")` 在 Anthropic provider conversion 后包含 `is_error=True`
- `test_failed_exchange_sanitization_preserves_later_error_summary`：失败 exchange 被清理，但后续 assistant 的自然语言错误总结仍保留
...
| 不做 ToolResult.output 文本猜测 | 按文本中 error/failed 等关键词推断 | 结构化 metadata 是 source of truth，避免误判正常输出 |
| `message_status()` 放低层模块 | 放在 agent sanitizer 或 message_rows | memory/session.py 也要使用，低层模块避免反向依赖 agent |
| 保留后续 assistant 错误总结 | 清理所有错误相关历史 | 只删除冗余协议 exchange，保留模型已作出的语义决策和解释 |
